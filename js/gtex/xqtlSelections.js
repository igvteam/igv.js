...
/**
 * A single object for handling XQTL selections.  Owned by "browser"
 */
class XQTLSelections {
    ...
    toJSON() {
        const obj = {}
        if (this.geneColors.size > 0) {
            obj.genes = Array.from(this.geneColors.keys())
        }
        if(this.snps.size > 0) {
    ...
    static fromJSON(json) {
        const xqtlSelections = new XQTLSelections()
        if(json.genes) {
            for(let g of json.genes) {
                xqtlSelections.addGene(g)
            }
        }
        if(json.snps) {
            for(let s of json.snps) {
                xqtlSelections.addSnp(s)
            }
        }
        if(json.qtl) {
            xqtlSelections.qtl = json.qtl
        }
        return xqtlSelections
    }
    ...
    clear() {
        this.geneColors = new Map()
        this.snps = new Set()
        this.qtl = null
    }

    isEmpty() {
        return this.geneColors.size === 0 &&  this.snps.size === 0 && this.qtl === null
    }
    ...
    addGene(geneName) {
        const genesCount = this.geneColors.size
        if (!this.geneColors.has(geneName.toUpperCase())) {
            const color = genesCount < brewer.length ? brewer[genesCount] : IGVColor.randomRGB(0, 255)
            this.geneColors.set(geneName.toUpperCase(), color)
        }
    }

    hasSnp(snp) {
        return this.snps.has(snp)
    }

    hasFeature(name) {
        return this.geneColors.has(name.toUpperCase())
    }
    ...
    colorForGene(geneName) {
        return this.geneColors.get(geneName.toUpperCase())
    }
}
