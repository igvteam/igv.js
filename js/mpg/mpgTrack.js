...
var igv = (function (igv) {
    ...
    igv.MpgTrack = function (url) {
        ...
        this.label = "Variants";
        this.id = "variants";
        this.height = 20;   // The preferred height
    }
     ...
     */
    igv.MpgTrack.prototype.draw = function (canvas, refFrame, bpStart, bpEnd, pixelWidth, pixelHeight, continuation) {

        var chr, py, len, py, endBP, xScale, variant, px, px1, pw, exonCount, step, cy, py, direction,
            exon, ePx, ePx1, ePw;
        ...
        this.featureSource.getFeatures(chr, bpStart, bpEnd, function (featureList) {
            ...
            if (featureList) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    exonCount = variant.exons ? variant.exons.length : 0;
                    ...
                    var geneColor;
                    if (igv.selection) geneColor = igv.selection.colorForGene(variant.name);
                    ...
                    if ((px1 - px) > 10 || geneColor) {
                        ...
                        canvas.fillText(variant.name, px + ((px1 - px) / 2), 2 * py, geneStyle, {rotate: {angle: 45}});
                    }
                }
            }
