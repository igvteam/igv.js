...
class TrackViewport extends Viewport {
    ...
    initializationHelper() {
        ...
        this.spinnerContainer.className = 'igv-loading-spinner-container';
        this.viewportElement.appendChild(this.spinnerContainer);
        ...
        if ('sequence' !== track.type) {
            this.zoomInNotice = this.createZoomInNotice(this.viewportElement);
        }

        if ("sequence" !== track.id) {
            this.trackLabel = document.createElement('div');
            this.trackLabel.className = 'igv-track-label';
            this.viewportElement.appendChild(this.trackLabel);
    ...
    setTrackLabel(label) {
        this.trackLabel.innerHTML = label;
        const txt = this.trackLabel.textContent;
        this.trackLabel.setAttribute('title', txt);
    }
    ...
    checkZoomIn() {
        ...
        if (this.trackView.track && "sequence" === this.trackView.track.type && this.referenceFrame.bpPerPixel > bppSequenceThreshold) {
            if (this.canvas) {
                this.viewportElement.removeChild(this.canvas);
        ...
        if (zoomedOutOfWindow()) {
            ...
            if (this.zoomInNotice) {
                this.zoomInNotice.style.display = 'block';
            }
            return false;
        } else {
            if (this.zoomInNotice) {
                this.zoomInNotice.style.display = 'none';
            }
            return true;
        }
    }
    ...
    async loadFeatures() {
        ...
        const pixelWidth = this.viewportElement.offsetWidth;
            ...
            if (features) {
                ...
                if (track.roiSets && track.roiSets.length > 0) {
                    for (let roiSet of track.roiSets) {
                        const roiFeatures = await roiSet.getFeatures(chr, bpStart, bpEnd, referenceFrame.bpPerPixel);
                        roiFeatures.push({ track: roiSet, features: roiFeatures });
                    }
                }
    ...
    repaint() {
        ...
        newCanvas.className = 'igv-canvas';
    ...
    savePNG() {
        ...
        const x = -this.canvas.offsetLeft * devicePixelRatio;
        ...
        const filename = (this.trackLabel.textContent ? this.trackLabel.textContent : "image") + ".png";
    ...
    renderSVGContext(context, { deltaX, deltaY }, includeLabel = true) {
        const zoomInNotice = this.zoomInNotice && this.zoomInNotice.style.display === "block";
        ...
        if (includeLabel && this.trackLabel && this.browser.doShowTrackLabels) {
            const { x, y, width, height } = DOMUtils.relativeDOMBBox(this.viewportElement, this.trackLabel);
    ...
    renderTrackLabelSVG(context, tx, ty, width, height) {
        ...
        const { width: stringWidth } = context.measureText(this.trackLabel.textContent);
        const dx = 0.25 * (width - stringWidth);
        const dy = 0.7 * (height - 12);
        context.fillText(this.trackLabel.textContent, dx, height - dy);
...
function formatPopoverText(nameValues) {
    const rows = nameValues.map(nameValue => {
        if (nameValue.name) {
            const str = `<span>${nameValue.name}</span>&nbsp;&nbsp;&nbsp;${nameValue.value}`;
