...
class AlignmentTrack extends TrackBase {
    ...
    draw(options) {
        ...
        // Set colorBy default if necessary
        if (!this.colorBy) {
        ...
        function drawSingleAlignment(alignment, y, alignmentHeight) {
            ...
            for (let b = 0; b < blocks.length; b++) {   // Can't use forEach here -- we need ability to break
                ...
                // Somewhat complex test, necessary to ensure gaps are drawn.
                // If this is not the last block, and the next block starts before the origin (off screen to left) then skip.
                ...
                if ((block.start + block.len) > bpEnd) {
                    // Do this after drawBlock to ensure gaps are drawn
                    break;
                }
            }
    ...
    menuItemList() {
        ...
        e.className = 'igv-track-menu-category';
        e.textContent = 'Color by:';
        ...
        e2.className = 'igv-track-menu-category';
        e2.textContent = 'Group by:';
        ...
        // Show all bases
        menuItems.push('<hr/>');
        const checkboxShowAllBases = createCheckbox("Show all bases", this.showAllBases);
        checkboxShowAllBases.addEventListener('change', function showAllBasesHandler() {
            this.alignmentTrack.showAllBases = !this.alignmentTrack.showAllBases;
            this.trackView.repaintViews();
        }.bind(this));
        menuItems.push({
            object: checkboxShowAllBases,
        ...
        // Show mismatches
        const checkboxShowMismatches = createCheckbox("Show mismatches", this.showMismatches);
        checkboxShowMismatches.addEventListener('change', function showMismatchesHandler() {
            this.alignmentTrack.showMismatches = !this.alignmentTrack.showMismatches;
            this.trackView.repaintViews();
        }.bind(this));
        menuItems.push({
            object: checkboxShowMismatches,
        ...
        // Insertions
        const checkboxShowInsertions = createCheckbox("Show insertions", this.showInsertions);
        checkboxShowInsertions.addEventListener('change', function showInsertionsHandler() {
            this.alignmentTrack.showInsertions = !this.alignmentTrack.showInsertions;
            this.trackView.repaintViews();
        }.bind(this));
        menuItems.push({
            object: checkboxShowInsertions,
        ...
        // Soft clips
        const checkboxShowSoftClips = createCheckbox("Show soft clips", this.showSoftClips);
        checkboxShowSoftClips.addEventListener('change', function showSoftClipsHandler() {
        ...
        if (this.hasPairs) {
            menuItems.push('<hr/>');
            const checkboxViewAsPairs = createCheckbox("View as pairs", this.viewAsPairs);
            checkboxViewAsPairs.addEventListener('change', function viewAsPairsHandler() {
        ...
        dml.className = 'igv-track-menu-category';
        dml.textContent = 'Display mode:';
     ...
     * @param menuItem
     * @param showCheck
     * @returns {{init: undefined, name: undefined, click: clickHandler, object: (HTMLElement|jQuery.fn.init)}}
     */
    colorByCB(menuItem, showCheck) {
        const e = createCheckbox(menuItem.label, showCheck);

        if (menuItem.key !== 'tag') {
            e.addEventListener('change', function clickHandler() {
                this.alignmentTrack.colorBy = menuItem.key;
                this.trackView.repaintViews();
            }.bind(this));

            return { name: undefined, object: e, click: undefined, init: undefined };
        } else {
            e.addEventListener('change', function dialogPresentationHandler(ev) {
            ...
            return { name: undefined, object: e, dialog: undefined, init: undefined };
        }
    }
    ...
    groupByCB(menuItem, showCheck) {
        ...
        e.addEventListener('change', function clickHandler(ev) {
     ...
     * the alignment color, unless the color option can result in split colors (separate color for each mate).
     *
     * @param alignment
     * @returns {string}
     */
