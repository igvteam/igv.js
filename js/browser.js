...
} from "./jbrowse/circularViewUtils.js";
...
import {
  sampleInfoTileWidth,
  sampleInfoTileXShim,
} from "./sample/sampleInfoConstants.js";
...
class Browser {
  ...
  constructor(config, parentDiv) {
    ...
    this.on("trackremoved", () => {
      const found = this.findTracks(
        (track) => typeof track.getSamples === "function"
      );
  ...
  createStandardControls(config) {
    ...
    this.chromosomeSelectWidget = new ChromosomeSelectWidget(
      this,
      $genomicLocation.get(0)
    );
    ...
    const searchIconContainer = DOMUtils.div({
    ...
    const $toggle_button_container = $(
      '<div class="igv-navbar-toggle-button-container">'
    );
    $navbarRightContainer.append($toggle_button_container);
    this.$toggle_button_container = $toggle_button_container;

    this.overlayTrackButton = new OverlayTrackButton(
      this,
      $toggle_button_container.get(0)
    );
    this.overlayTrackButton.setVisibility(false);

    this.multiTrackSelectButton = new MultiTrackSelectButton(
      this,
      $toggle_button_container.get(0)
    );
    ...
    this.cursorGuideButton = new CursorGuideButton(
      this,
      $toggle_button_container.get(0)
    );

    this.centerLineButton = new CenterLineButton(
      this,
      $toggle_button_container.get(0)
    );

    this.setTrackLabelVisibility(config.showTrackLabels);
    this.trackLabelControl = new TrackLabelControl(
      $toggle_button_container.get(0),
      this
    );

    // ROI Control
    this.roiTableControl = new ROITableControl(
      $toggle_button_container.get(0),
      this
    );

    this.sampleInfoControl = new SampleInfoControl(
      $toggle_button_container.get(0),
      this
    );

    this.sampleNameControl = new SampleNameControl(
      $toggle_button_container.get(0),
      this
    );

    if (true === config.showSVGButton) {
      this.saveImageControl = new SaveImageControl(
        $toggle_button_container.get(0),
        this
      );
    }
    ...
    this.dataRangeDialog.$container.get(
      0
    ).id = `igv-data-range-dialog-${DOMUtils.guid()}`;
  ...
  isMultiLocusWholeGenomeView() {
    if (
      undefined === this.referenceFrameList ||
      1 === this.referenceFrameList.length
    ) {
      return false;
    }
  ...
  currentLoci() {
    ...
    if (
      undefined === this.referenceFrameList ||
      0 === this.referenceFrameList.length
    ) {
      return "";
  ...
  static async loadSessionFile(options) {
    ...
    if (
      options.url &&
      StringUtils.isString(options.url) &&
  ...
  async loadSessionObject(session) {
    ...
    if (false !== session.showRuler) {
      this.trackViews.push(
        new TrackView(this, this.columnContainer, new RulerTrack(this))
      );
    }
    ...
    if (session.roi) {
      ...
      const named = roiSetList.filter(
        ({ name }) => name !== undefined && name.length > 0
      );

      roiTableConfig.columnFormat = ROITable.getColumnFormatConfiguration(
        named.length > 0
      );
      ...
      roiTableConfig.columnFormat =
        ROITable.getColumnFormatConfiguration(false);
    ...
    const trackConfigurations = session.tracks
      ? genomeTracks.concat(session.tracks)
      : genomeTracks;

    // Ensure that we always have a sequence track with no explicit URL (=> the reference genome sequence track)
    const pushSequenceTrack =
      trackConfigurations.filter(
        (track) => "sequence" === track.type && !track.url && !track.fastaURL
      ).length === 0;
    ...
    const localTrackFileNames = trackConfigurations
      .filter((config) => undefined !== config.file)
      .map(({ file }) => file);

    const localIndexFileNames = trackConfigurations
      .filter((config) => undefined !== config.indexFile)
      .map(({ indexFile }) => indexFile);
    ...
    if (localTrackFileNames.length > 0) {
      alert(
        `Local files cannot be loaded automatically.\nThis session contains references to these local files:\n${localTrackFileNames
          .map((str) => `    ${str}`)
          .join("\n")}`
      );
    }

    const nonLocalTrackConfigurations = trackConfigurations.filter(
      (config) => undefined === config.file
    );
    ...
    for (let rtv of this.trackViews.filter(
      (tv) => tv.track.type === "ruler" || tv.track.type === "ideogram"
    )) {
      rtv.updateViews();
    }
  ...
  cleanHouseForSession() {
    ...
    // discard all columns
    const elements = this.columnContainer.querySelectorAll(
      ".igv-axis-column, .igv-column-shim, .igv-column, .igv-sample-info-column, .igv-sample-name-column, .igv-scrollbar-column, .igv-track-drag-column, .igv-gear-menu-column"
    );
  ...
  async loadReference(genomeConfig, initialLocus) {
    ...
    const genomeChange =
      undefined === this.genome || this.genome.id !== genome.id;
  ...
  updateNavbarDOMWithGenome(genome) {
    let genomeLabel =
      genome.id && genome.id.length < 20
        ? genome.id
        : `${genome.id.substring(0, 8)}...${genome.id.substring(
            genome.id.length - 8
          )}`;
  ...
  async loadGenome(idOrConfig) {
    ...
    const isHubGenome =
      ...
      (idOrConfig.url &&
        ...
        idOrConfig.url.endsWith("/hub.txt"));
    if (isHubGenome) {
      const hub = await Hub.loadHub(
        idOrConfig.hubURL || idOrConfig.url,
        idOrConfig
      );
      genomeConfig = hub.getGenomeConfig();
    } else if (
      StringUtils.isString(idOrConfig) ||
      !(
        idOrConfig.url ||
        idOrConfig.fastaURL ||
        idOrConfig.twoBitURL ||
        idOrConfig.gbkURL
      )
    ) {
    ...
    const pushSequenceTrack =
      tracks.filter((track) => track.type === "sequence").length === 0;
  ...
  updateUIWithReferenceFrameList() {
    ...
    const isWGV =
      ...
      GenomeUtils.isWholeGenomeView(referenceFrameList[0].chr);
    ...
    if (
      this.doShowCursorGuide &&
      GenomeUtils.isWholeGenomeView(referenceFrameList[0].chr)
    ) {
      this.cursorGuideButton.boundMouseClickHandler();
    }

    this.setCenterLineAndCenterLineButtonVisibility(
      GenomeUtils.isWholeGenomeView(referenceFrameList[0].chr)
    );
  }
  ...
  setROITableVisibility(isVisible) {
    true === isVisible
      ? this.roiManager.presentTable()
      : this.roiManager.dismissTable();
  }
  ...
  async _loadTrack(config) {
    ...
    try {
      ...
      if (config.hidden) {
        ...
        await featureSource.getFeatures({
          chr: "1",
          start: 0,
      ...
      msg = `${msg} : ${
        FileUtils.isFile(config.url) ? config.url.name : config.url
      }`;
  ...
  async createTrack(config) {
    ...
    if ("sampleinfo" === type) {
      ...
      if (undefined === track) {
        this.alert.present(
          new Error(
            `Error creating track.  Could not determine track type for file: ${
              config.url || config
            }`
          ),
          undefined
        ...
        if (config.roi && config.roi.length > 0) {
          track.roiSets = config.roi.map(
            (r) => new TrackROISet(r, this.genome)
          );
        }
  ...
  reorderTracks() {
    ...
    for (let {
      ...
      this.columnContainer
        .querySelector(".igv-sample-info-column")
        .appendChild(sampleInfoViewport.viewport);

      this.columnContainer
        .querySelector(".igv-sample-name-column")
        .appendChild(sampleNameViewport.viewport);

      this.columnContainer
        .querySelector(".igv-scrollbar-column")
        .appendChild(outerScroll);

      this.columnContainer
        .querySelector(".igv-track-drag-column")
        .appendChild(dragHandle);

      this.columnContainer
        .querySelector(".igv-gear-menu-column")
        .appendChild(gearContainer);
    }
  }

  getTrackOrder() {
    return this.trackViews
      .filter((tv) => tv.track && tv.track.name)
  ...
  getTrackURLs() {
    return new Set(
      this.tracks
        .filter(
          (track) => track.config && StringUtils.isString(track.config.url)
        )
  ...
  async layoutChange() {
    const status = this.referenceFrameList.find(
      (referenceFrame) => referenceFrame.bpPerPixel < 0
    );

    if (status) {
      const viewportWidth = this.calculateViewportWidth(
        this.referenceFrameList.length
      );
      for (let referenceFrame of this.referenceFrameList) {
        referenceFrame.bpPerPixel =
          (referenceFrame.end - referenceFrame.start) / viewportWidth;
      }
    }
    ...
    if (this.referenceFrameList) {
      const isWGV =
        this.isMultiLocusWholeGenomeView() ||
        GenomeUtils.isWholeGenomeView(this.referenceFrameList[0].chr);
      navbarDidResize(this, this.$navigation.width(), isWGV);
    }
  ...
  async updateViews() {
    ...
    if (this.dragObject) {
      ...
      if (Object.entries(groupAutoscaleTrackViews).length > 0) {
        for (const [group, trackViews] of Object.entries(
          groupAutoscaleTrackViews
        )) {
          const featureArray = await Promise.all(
            trackViews.map((trackView) => trackView.getInViewFeatures())
          );
          ...
          await Promise.all(
            trackViews.map((trackView) => trackView.updateViews())
          );
        }
      }
  ...
  updateLocusSearchWidget() {
    ...
    for (let referenceFrame of referenceFrameList) {
      referenceFrame.end =
        referenceFrame.start + referenceFrame.bpPerPixel * width;
    }

    if (this.chromosomeSelectWidget) {
      this.chromosomeSelectWidget.select.value =
        referenceFrameList.length === 1 ? this.referenceFrameList[0].chr : "";
    }

    const loc = this.referenceFrameList
  ...
  calculateViewportWidth(columnCount) {
    ...
    width -=
      igv_axis_column_width +
      this.getSampleInfoViewportWidth() +
      this.getSampleNameViewportWidth() +
      igv_scrollbar_outer_width +
      igv_track_manipulation_handle_width +
      igv_track_gear_menu_column_width;
  ...
  async zoomWithScaleFactor(
    scaleFactor,
    centerBPOrUndefined,
    referenceFrameOrUndefined
  ) {
    if (!this.referenceFrameList) return;
    ...
    let referenceFrames = referenceFrameOrUndefined
      ? [referenceFrameOrUndefined]
      : this.referenceFrameList;

    for (let referenceFrame of referenceFrames) {
      referenceFrame.zoomWithScaleFactor(
        this,
        scaleFactor,
        viewportWidth,
        centerBPOrUndefined
  ...
  async addMultiLocusPanel(chr, start, end, referenceFrameLeft) {
    ...
    const scaleFactor =
      this.calculateViewportWidth(this.referenceFrameList.length) /
      this.calculateViewportWidth(1 + this.referenceFrameList.length);
    for (let refFrame of this.referenceFrameList) {
      refFrame.bpPerPixel *= scaleFactor;
    }

    const bpp = (end - start) / viewportWidth;
    const newReferenceFrame = new ReferenceFrame(
      this.genome,
      chr,
      start,
      end,
    ...
    const indexLeft = referenceFrameLeft
      ? this.referenceFrameList.indexOf(referenceFrameLeft)
      : this.referenceFrameList.length - 1;
    const indexRight = 1 + indexLeft;

    // TODO -- this is really ugly
    const { $viewport } = this.trackViews[0].viewports[indexLeft];
    const viewportColumn = viewportColumnManager.insertAfter(
      $viewport.get(0).parentElement
    );
  ...
  createCenterLineList(columnContainer) {
    ...
    for (let i = 0; i < viewportColumns.length; i++) {
      centerLineList.push(
        new ViewportCenterLine(
          this,
          this.referenceFrameList[i],
          viewportColumns[i]
        )
      );
    }
  ...
  async gotoMultilocusPanel(referenceFrame) {
    ...
    this.columnContainer
      .querySelectorAll(".igv-column-shim")
      .forEach((shim) => shim.remove());

    // Discard viewports
    for (let trackView of this.trackViews) {
      const retain = trackView.viewports[referenceFrameIndex];
      trackView.viewports
        .filter((viewport, i) => i !== referenceFrameIndex)
        .forEach((viewport) => viewport.dispose());
    ...
    this.trackViews.forEach(({ viewports }) =>
      viewports.forEach((viewport) => viewport.setWidth(viewportWidth))
    );
  ...
  async search(string, init) {
    ...
    if (loci && loci.length > 0) {
      ...
      this.referenceFrameList = createReferenceFrameList(
        ...
        this.flanking,
        this.minimumBases(),
        this.calculateViewportWidth(loci.length),
        this.isSoftclipped()
      );
      ...
      this.columnContainer
        .querySelectorAll(".igv-column-shim, .igv-column")
        .forEach((el) => el.remove());

      // Insert viewport columns preceding the sample info column
      viewportColumnManager.insertBefore(
        this.columnContainer.querySelector(".igv-sample-info-column"),
        this.referenceFrameList.length
      );
      this.fireEvent("didchangecolumnlayout");
      ...
      for (let trackView of this.trackViews) {
        trackView.addDOMToColumnContainer(
          this,
          this.columnContainer,
          this.referenceFrameList
        );
  ...
  getSampleInfoColumnWidth() {
    if (!this.sampleInfo.attributeCount) {
      ...
      if (status) {
        return (
          this.sampleInfo.attributeCount * sampleInfoTileWidth +
          sampleInfoTileXShim
        );
      } else {
        return 0;
  ...
  toJSON() {
    ...
    if (json.reference.fastaURL instanceof File) {
      ...
      throw new Error(
        `Error. Sessions cannot include local file references ${json.reference.fastaURL.name}.`
      );
    } else if (json.reference.indexURL instanceof File) {
      // Test specifically for File.  Other types of File-like objects might be savable) {
      throw new Error(
        `Error. Sessions cannot include local file references ${json.reference.indexURL.name}.`
      );
    }
    ...
    if (localFileDetections.length > 0) {
      alert(
        `This session includes reference(s) to local file(s):\n${localFileDetections
          .map((str) => `    ${str}`)
          .join(
            "\n"
  ...
  cancelTrackPan() {
    ...
    if (
      dragObject &&
      dragObject.viewport.referenceFrame.start !== dragObject.start
    ) {
      this.updateViews();
  ...
  isSoftclipped() {
    const result = this.trackViews.find(
      (tv) => tv.track.showSoftClips === true
    );
  ...
  removeRootMouseLeaveHandler() {
    this.root.removeEventListener(
      "mouseleave",
      this.boundRootMouseLeaveHandler
    );
  }
  ...
  addColumnContainerEventHandlers() {
    ...
    this.columnContainer.addEventListener(
      "mousemove",
      this.boundColumnContainerMouseMoveHandler
    );
    this.columnContainer.addEventListener(
      "touchmove",
      this.boundColumnContainerTouchMoveHandler
    );

    this.columnContainer.addEventListener(
      "mouseleave",
      this.boundColumnContainerMouseLeaveHandler
    );

    this.columnContainer.addEventListener(
      "mouseup",
      this.boundColumnContainerMouseUpHandler
    );
    this.columnContainer.addEventListener(
      "touchend",
      this.boundColumnContainerTouchEndHandler
    );
  }

  removeColumnContainerEventHandlers() {
    this.columnContainer.removeEventListener(
      "mousemove",
      this.boundColumnContainerMouseMoveHandler
    );
    this.columnContainer.removeEventListener(
  ...
  createCircularView(container, show) {
    ...
    this.circularViewControl = new CircularViewControl(
      this.$toggle_button_container.get(0),
      this
    );
    this.circularView.setAssembly({
      name: this.genome.id,
...
async function resize() {
  ...
  for (let referenceFrame of this.referenceFrameList) {
    ...
    if (GenomeUtils.isWholeGenomeView(chr) || viewportWidthBP > bpLength) {
      ...
      referenceFrame.end =
        referenceFrame.start + referenceFrame.toBP(viewportWidth);
    }
...
function handleMouseMove(e) {
  ...
  if (this.vpMouseDown) {
    ...
    const horizontal =
      Math.abs(x - this.vpMouseDown.mouseDownX) >
      Math.abs(y - this.vpMouseDown.mouseDownY);

    if (!this.dragObject && !this.isScrolling) {
      if (horizontal) {
        if (
          this.vpMouseDown.mouseDownX &&
          Math.abs(x - this.vpMouseDown.mouseDownX) >
            this.constants.dragThreshold
        ) {
          this.dragObject = { viewport, start: referenceFrame.start };
        }
      } else {
        if (
          this.vpMouseDown.mouseDownY &&
          Math.abs(y - this.vpMouseDown.mouseDownY) >
            this.constants.scrollThreshold
        ) {
